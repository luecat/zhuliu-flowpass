...
export async function GET(request: Request, context: { params: Promise<{ caseId: string }> }): Promise<Response> {
  ...
  const rows = runtime.database.prepare(`
    SELECT a.id, a.status, a.severity, a.public_summary, a.public_guidance, a.created_at, a.resolved_at,
           si.title AS incident_title
    FROM alerts a
    JOIN cases c ON c.id = a.case_id
    LEFT JOIN incident_matches im ON im.id = a.incident_match_id
    LEFT JOIN security_incidents si ON si.id = im.security_incident_id
    WHERE a.case_id = ?
      AND c.applicant_id = ?
      AND a.status IN ('open', 'acknowledged', 'resolved')
    ORDER BY a.created_at DESC, a.id DESC
  `).all(caseId, session.applicantId) as Array<{
    id: string;
    status: string;
    severity: string;
