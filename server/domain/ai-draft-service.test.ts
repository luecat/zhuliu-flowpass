...
describe('ai draft admission boundary', () => {
  ...
  it('persists only IDs/control metadata in the durable payload and enforces the injectable token budget', () => {
    ...
    cases.saveAnswers({ applicantId: IDS.applicant, caseId: created.case.id, answers: { material: '機密答案 sentinel', aiPurpose: '整理', sensitiveData: '健康', destinationAndAudience: '團隊' }, ifMatch: '"1"', idempotencyKey: 'answers' });
