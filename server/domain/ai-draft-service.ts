...
export function createAiDraftService(options: AiDraftServiceOptions): AiDraftService {
  ...
  function enqueueInTransaction(input: { applicantId: string; caseId: string; operation: 'draft' | 'revise'; expectedRowVersion?: number; retryNonce?: string }): { job: DurableJob; inputHash: string; inputTokens: number } {
    ...
    let uniqueKey = `ai-draft:${input.caseId}:${prepared.answerVersionId}:${prepared.passportVersionId ?? 'none'}:${input.operation}${retrySuffix}`;
    ...
    const enqueueJob = (key: string) => jobs.enqueue({ systemId: 'public-ai-admission' }, {
      jobType: 'ai_draft',
      uniqueKey: key,
    ...
    let job = enqueueJob(uniqueKey);
    // A terminal failure still occupies the original unique key. Reusing it
    // returns that dead job, and the wizard immediately shows 處理失敗 without
    // ever calling the model again.
    if (job.state === 'failed_terminal') {
      uniqueKey = `${uniqueKey}:retry:${uuidv7()}`;
      job = enqueueJob(uniqueKey);
    }
