...
export async function POST(request: Request): Promise<Response> {
  ...
  const replay = readApplicantMutation({ database: runtime.database, crypto: runtime.crypto, applicantId: csrf.applicantId, method: 'POST', normalizedRoute: '/api/v1/cases', idempotencyKey: key, requestProjection: { programCycleId: parsed.data.programCycleId }, now });
  ...
  try {
    const result = createCaseService({ database: runtime.database, crypto: runtime.crypto, clock: runtime.clock, requestIdGenerator: runtime.requestIdGenerator }).create({ applicantId: csrf.applicantId, programCycleId: parsed.data.programCycleId, idempotencyKey: key, requestId });
