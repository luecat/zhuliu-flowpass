...
function Panel({ item, busy, error, close, clearError, update }: { item: Case; busy: boolean; error: string; close: () => void; clearError: () => void; update: (review: Review, decision: ReviewDecision) => Promise<void> }) {
  ...
  const decision: ReviewDecision = review?.supplement
    ...
    : {
        ...(review?.reasonLabel ? { reason: reason.trim() } : {}),
  ...
  return <aside className="panel" aria-labelledby="case-title">
    ...
    {availableReviews.length === 0 ? <section className="review review-empty"><h3>更新案件狀態</h3><p>這個狀態目前沒有可執行的審核動作。</p></section> : <form className="review" onSubmit={submit}>
      ...
      {review?.reasonLabel && <div className="field"><label htmlFor="reason">{review.reasonLabel}（必填）</label><textarea id="reason" rows={3} value={reason} onChange={(event) => setReason(event.target.value)} placeholder={review.reasonPlaceholder} /></div>}
...
function App() { const [session, setSession] = useState<Session | null>(null), [view, setView] = useState<'login' | 'recovery' | 'password' | 'dashboard'>('login'), [cases, setCases] = useState<Case[]>([]), [selected, setSelected] = useState<Case | null>(null), [managing, setManaging] = useState<Case | null>(null), [busy, setBusy] = useState(false), [loading, setLoading] = useState(true), [error, setError] = useState(''), [notice, setNotice] = useState('');
