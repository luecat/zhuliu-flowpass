...
function stubAdmin(cases: ListedCase[]) {
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init: RequestInit = {}) => {
    ...
    if (path.startsWith('/admin/v1/cases/') && path.endsWith('/evaluations')) return json({ evaluations: [] });
    if (path.startsWith('/admin/v1/cases/') && path.endsWith('/purchase-details')) return json({ purchaseDetails: null });
...
async function openCaseActions(caseCode: string) {
  fireEvent.click(await screen.findByRole('button', { name: `開啟案件 ${caseCode}` }));
  await screen.findByRole('combobox', { name: '審核動作' });
}
...
describe('FlowPass admin attachment panel', () => {
  ...
  it('loads and renders the selected case attachments with document type labels', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (path === `/admin/v1/cases/${CASE_ID}/evaluations`) return json({ evaluations: [] });
      if (path === `/admin/v1/cases/${CASE_ID}/purchase-details`) return json({ purchaseDetails: null });
    ...
    expect(await screen.findByText('購買憑證或發票')).toBeInTheDocument();
    expect(screen.queryByText('九月發票.pdf')).not.toBeInTheDocument();
...
describe('FlowPass admin review actions', () => {
  ...
  it('shows only legal actions for the selected case state', async () => {
    ...
    await openCaseActions(CASE_CODE);
    ...
    expect(screen.queryByRole('option', { name: '退回修正' })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: '← 返回案件總覽' }));
    ...
    expect(screen.getByText('目前狀態「已結案」沒有可執行的處理方式。')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '← 返回案件總覽' })).toBeInTheDocument();
  });
  ...
  it('starts review without requiring an applicant-visible reason', async () => {
    ...
    const start = screen.getByRole('button', { name: '完成' });
  ...
  it('confirms a complete supplement request before sending the backend contract', async () => {
    ...
    const openConfirmation = screen.getByRole('button', { name: '完成' });
    expect(openConfirmation).toBeEnabled();
    fireEvent.click(openConfirmation);
    const dialog = await screen.findByRole('dialog', { name: '確認要求補件' });
    ...
    await waitFor(() => expect(screen.queryByRole('dialog', { name: '確認要求補件' })).not.toBeInTheDocument());
    expect(document.activeElement).toBe(openConfirmation);

    fireEvent.click(openConfirmation);
    const reopenedDialog = await screen.findByRole('dialog', { name: '確認要求補件' });

    fireEvent.click(within(reopenedDialog).getByRole('button', { name: '返回修改' }));
    expect(screen.queryByRole('dialog', { name: '確認要求補件' })).not.toBeInTheDocument();
    expect(reviewBodies).toHaveLength(0);

    fireEvent.click(screen.getByRole('button', { name: '完成' }));
    fireEvent.click(await screen.findByRole('button', { name: '確認要求補件' }));
    await waitFor(() => expect(reviewBodies).toHaveLength(1));
