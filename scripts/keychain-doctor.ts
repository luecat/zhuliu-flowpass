...
export const LOGICAL_SECRETS = ['line-channel-access-token', 'line-channel-secret', 'line-login-channel-secret', 'lm-studio-api-token', 'vault-master-key-v1', 'session-hmac-key-v1', 'backup-master-key-v1'] as const;
...
if (import.meta.url === `file://${process.argv[1]}`) void keychainDoctor().then((result) => console.log(JSON.stringify(result))).catch(() => { process.exitCode = 1; });
