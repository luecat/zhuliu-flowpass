...
describe('PUT /api/v1/cases/:caseId/answers boundary', () => {
  ...
  const makeRequest = (extra: Record<string, string> = {}) => new Request('http://127.0.0.1:38100/api/v1/cases/c/answers', { method: 'PUT', headers: { origin: 'http://127.0.0.1:38100', cookie: 'flowpass_session=s; flowpass_csrf=c', 'x-flowpass-csrf': 'c', 'content-type': 'application/json', 'idempotency-key': 'answers-test', 'if-match': '"1"', ...extra }, body: JSON.stringify({ material: '資料', aiPurpose: '整理', sensitiveData: '無', destinationAndAudience: '團隊', applicantName: '測試申請人' }) });
