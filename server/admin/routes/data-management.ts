...
export function createDataManagementRoutes(database: FlowPassDatabase, dependencies: DataManagementRouteDependencies) {
  ...
  app.get('/admin/v1/data/passports/:caseId', (context) => {
    ...
    const snapshot = getAdminPassportDataSnapshot(database, dependencies.crypto, context.req.param('caseId'));
  ...
  app.patch('/admin/v1/data/passports/:caseId/fields', async (context) => {
    ...
    try {
      const result = editAdminPassportField({ ...body, database, crypto: dependencies.crypto, caseId: context.req.param('caseId'), adminId: auth.adminId, requestId: crypto.randomUUID() });
  ...
  app.post('/admin/v1/data/passports/:caseId/purge-preview', (context) => {
    ...
    try { return context.json({ data: previewPassportPurge({ database, crypto: dependencies.crypto, documentVault: dependencies.documentVault, dataRoot: dependencies.dataRoot, backupRoot: dependencies.backupRoot }, context.req.param('caseId')) }); }
  ...
  app.post('/admin/v1/data/passports/:caseId/purge-authorizations', async (context) => {
    ...
    try {
      const data = await authorizePassportPurge({ database, crypto: dependencies.crypto, documentVault: dependencies.documentVault, dataRoot: dependencies.dataRoot, backupRoot: dependencies.backupRoot }, { caseId: context.req.param('caseId'), adminId: auth.adminId, ...body });
  ...
  app.post('/admin/v1/data/passports/:caseId/purge', async (context) => {
    ...
    try {
      const data = await executePassportPurge({ database, crypto: dependencies.crypto, documentVault: dependencies.documentVault, dataRoot: dependencies.dataRoot, backupRoot: dependencies.backupRoot }, { caseId: context.req.param('caseId'), adminId: auth.adminId, token: body.token });
